...
`krx_options`가 "KRX 서비스 미신청"으로 실패하면 키 문제가 아니다(2026-09-05 확인: 같은 키로
선물·주식·지수는 정상). KRX OpenAPI 포털에서 **파생상품 옵션 일별매매정보** 서비스를 따로
신청해야 한다. 신청 전까지 put/call 비율은 웹으로 조사하라.
