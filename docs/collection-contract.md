...
| `kcif@kcif.or.kr` | 경제 | 국제금융센터. **수신되나 API 미반환 — 웹이 정본**(아래 절). API가 돌려주면 Tier 1 |
...
### 국제금융센터(KCIF) — **Gmail에 수신돼 있으나 커넥터가 돌려주지 않는다. 웹이 정본**
...
**메일은 `gihojeong@gmail.com`에 정상 수신돼 있다**(사용자 스크린샷: 9/25 받은편지함,
발신 `국제금융센터`, 제목 `[9월 25일 daily] 미국 30년물 국채금리, 장중 5.50%까지…`, 별표).
**그런데 Gmail 커넥터 검색이 이 메일을 돌려주지 않는다.** 실측(2026-09-26):

- 스크린샷 상위 20행과 `in:inbox` API 결과를 한 줄씩 대조 → **18행 반환, 2행 누락**
  (`국제금융센터`, `피렌체의식탁`). 앞뒤 메일은 정상 반환되고 그 자리만 비어 있다.
- 라벨 통계 `INBOX 264건`은 웹 화면 "264개 중"과 **일치** — 메일함은 그 메일을 세고 있다.
- `is:starred newer_than:3d` → 별표 KCIF 누락 / 본문 정확검색 `30년물 국채금리 5.50%` → 0건
- `from:kcif.or.kr in:anywhere` → **2025-07-14 3건만** 반환(최근 것은 전부 미반환)

→ **판정: 커넥터 수준에서 일부 메일이 검색 결과에서 제외된다. 원인은 이쪽에서 특정 불가.**
  쿼리를 바꿔도 복구되지 않는다(발신자·본문·별표·기간·in:anywhere 전부 시도).
★이전 판정("구독 없음" → "구독 끊김" → "다른 주소로 수신")은 **셋 다 틀렸다.**
...
- 메일 API가 KCIF를 돌려주면 그대로 Tier 1으로 쓰되, **안 돌려줘도 웹 경로는 생략하지 마라.**

★★교훈 — **Gmail API 결과는 불완전하다. "0건"은 "없음"이 아니다.**
1. API에서 0건이면 **"API 미반환"**으로 적어라. "미수신", "구독 없음", "메일 없음"으로 쓰지 마라.
2. KCIF처럼 **웹에 원본이 있는 소스는 웹을 정본**으로 둔다. 메일은 보조다.
3. 웹 원본이 없는 메일 소스(ARK·SemiAnalysis 등)가 0건이면 해당 발행처 웹(substack 아카이브,
   arkinvest.com)에서 그 기간 발행 여부를 확인하고 결과를 함께 적어라.
4. **"찾지 못했다"를 "존재하지 않는다"로 쓰는 것이 이 절에서 세 번 반복된 실패다.**
