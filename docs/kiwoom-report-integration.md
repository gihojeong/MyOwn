...
# 개장 전 브리핑 (평일 06:00) — 34건. --date에 직전 거래일을 넘긴다
...
**의도적으로 넣지 않은 것**: 프로그램매매 추이(`ka90010`)는 필수 파라미터
`mrkt_tp`("프로그램 시장코드")의 허용값이 스펙에 열거돼 있지 않다. 추측해서 넣기보다
첫 실행 후 `--call ka90010`로 값을 확인해 추가하는 편이 낫다.

## 검증 상태

- 오류 경로(자격증명 누락·placeholder·잘못된 mode·네트워크 차단): 확인함
- preset 작업 테이블 구성(close 44 / premarket 34 / weekly 24, 이름 중복 없음): 확인함
- **실제 API 응답: 미검증** — 네트워크가 열리기 전까지 호출 자체가 불가능하다.
  허용 후 `--check` → `--preset close` 순으로 한 번 돌려 `summary.failed`를 확인해야 한다.
